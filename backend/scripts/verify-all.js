...
async function runScenario1_DuplicateDetection() {
    ...
    const mainAsset = assets.find((a) => a.name === '官方主站');
    const duplicateData = {
        title: '登录页面存在SQL注入漏洞',
...
async function runScenario2_CriticalNoFixNoBounty() {
    ...
    const reportsRes = await makeRequest('GET', '/reports?status=FIXING&severity=HIGH', undefined, adminToken);
    ...
    const bountyRes = await makeRequest('POST', `/reports/${fixingReport.id}/request-bounty`, { bountyAmount: 10000 }, triagerToken);
...
async function runScenario3_OnlyResearcherCanVerify() {
    ...
    const reportsRes = await makeRequest('GET', '/reports?status=VERIFIED&severity=CRITICAL', undefined, adminToken);
    let verifiedReport = reportsRes.data?.data?.[0];
    if (!verifiedReport) {
        console.log('   ⚠️  需要先创建一个处于 RETESTING 状态的报告');
        const submittedRes = await makeRequest('GET', '/reports?status=SUBMITTED', undefined, adminToken);
    ...
    const verifyRes = await makeRequest('POST', `/reports/${verifiedReport.id}/verify-fix`, { isVerified: true, comment: '测试验证' }, researcher2Token);
