...
async function getOriginalReportId(token, assetId) {
    const res = await makeRequest('GET', `/reports?assetId=${assetId}&isMerged=false`, undefined, token);
    if (res.status !== 200 || !res.data?.success) {
        throw new Error(`Get reports failed: ${JSON.stringify(res.data)}`);
    }
    const reports = res.data.data;
    const sqlInjectionReport = reports.find((r) => r.title.includes('SQL注入') && !r.isMerged);
...
async function verifyScenario() {
    ...
    try {
        ...
        const originalReportRes = await makeRequest('GET', `/reports/${originalReportId}`, undefined, adminToken);
...
username=test' OR 1=1-- &password=123456`,
        ...
        if (duplicateInfo?.isDuplicate === true) {
            ...
            console.log(`      📋 匹配原因:`);
            duplicateInfo.matchReasons.forEach((reason) => {
                console.log(`         - ${reason}`);
            });
        }
